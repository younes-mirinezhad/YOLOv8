...
Detector_ONNXRUNTIME::Detector_ONNXRUNTIME(QObject *parent) : Detector{parent} {
    qDebug() << Q_FUNC_INFO;
}

bool Detector_ONNXRUNTIME::LoadModel(QString &modelPath)
{
    ...
    try {
        ...
        _OrtSession = new Ort::Session(_OrtEnv, modelPath.toStdString().c_str(), _OrtSessionOptions);
#else
        _OrtSession = new Ort::Session(_OrtEnv, modelPath.toStdString().c_str(), _OrtSessionOptions);
#endif
        ...
        if (_outputNodesNum != 1) {
            qDebug() << "This model has "
                     << _outputNodesNum
                     << "output, which is not a detection model.Please check your model name or path!";
            return false;
        }
